; *** NetLogo 4.1beta3 Code ChallengeOne Part 1***
...
patches-own [height]     ; jeder patch kennt seine Höhe
globals     [maxheight]  ; für alle Routinen soll die maximale Höhe bekannt sein
...
to setup
  ...
  ask n-of anzahlhuegel patches [ set height (random-float 100 + 90)]

  ;; Verteile von diesen patches aus den Höhenwert in die Nachbarschaft
  repeat (max-pxcor) [ diffuse height 1]

  ;; identifiziere die höchste Höhe
  set maxheight max [height] of patches

  ;;skaliere die Farbe Braun zwischen dem Wert 0.5*maxheight und maxheight
  ;;skaliere die Farbe Grün zwischen dem Wert 0 und 0.5*maxheight
  ask patches [ ifelse (height > maxheight / 2) [set pcolor scale-color brown height  (maxheight + 1) (maxheight * .5) ]
                                            [set pcolor scale-color green height  -1 (maxheight * 0.5)  ]]
...
;; go-turtles8 ist die Referenzroutine d.h. diese wird weiter Verwendung finden
...
;; schau in der 8-er Nachbarschaft nach dem höchsten Wert und richte dich dorthin aus
  face max-one-of neighbors [height]
  ifelse draw-path?
    [ pen-down ]
    [ pen-up]
...
;; gehe einen patch vorwärts
  ...
  plot mean [height] of turtles
end
;;----------------------End go-turtles8 ---


;; Endlosroutine (siehe Button) läßt die turtles nach dem höchsten Punkt suchen
;; ist identisch mit go-turtles8 mit dem Unterschied dass der Befehl vom OBSERVER aufgerufen wird
to go-observer8

;; schau in der 8-er Nachbarschaft nach dem höchsten Wert und richte dich dorthin aus
 ask turtles [ face max-one-of neighbors [height]
               ;; gehe einen patch vorwärts
        ifelse draw-path?
    [ pen-down ]
    [ pen-up]
...
;; schau in der 4-er Nachbarschaft nach dem höchsten Wert und richte dich dorthin aus
 ask turtles [ face max-one-of neighbors4 [height]
      ifelse draw-path?
    [ pen-down ]
    [ pen-up]
;; gehe die im Slider eingestellte Zahl an patches vorwärts
               fd 1
             ]
end
